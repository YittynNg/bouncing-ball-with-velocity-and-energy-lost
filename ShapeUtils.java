...
public class ShapeUtils {
    ...
    public static boolean isPtInCircle(Point ptIn, Point cirUpperLeft, int cirWidth, int cirHeight) {
        Point ptCenter = squareUpperLeftToCenterPt(cirUpperLeft, cirWidth, cirHeight);   //% calculate centre point of ball
        double radius = cirWidth / 2;
        return (distanceFormulua(ptIn, ptCenter) <= radius);    
        //% return boolean whether the distance mouse click is smaller than radius (holding the ball)
    }
